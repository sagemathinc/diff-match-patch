...
export class DiffMatchPatch {
  ...
  public diff_main(
    ...
    const diffs: Diff[] = this.diff_compute_(
      text1,
      text2,
      checklines,
      deadline,
  ...
  public diff_linesToChars(
    ...
    const chars1 = this.diff_linesToCharsMunge_(
      text1,
      lineArray,
      lineHash,
      40000,
    );
    const chars2 = this.diff_linesToCharsMunge_(
      text2,
      lineArray,
      lineHash,
      65535,
            ...
            for (let y = 0; y < patches[x].diffs.length; y++) {
              ...
              if (mod[0] === DiffOperation.DIFF_INSERT) {
                ...
                text =
                  text.substring(0, startLoc + index2) +
                  text.substring(
                    startLoc + this.diff_xIndex(diffs, index1 + mod[1].length),
                  );
              }
  ...
  public patch_addPadding(patches: PatchObject[]): string {
    ...
    if (
      diffs.length === 0 ||
      diffs[diffs.length - 1][0] !== DiffOperation.DIFF_EQUAL
    ) {
      // Add nullPadding equality.
      ...
      while (bigpatch.diffs.length !== 0) {
        ...
        while (
          ...
          if (diffType === DiffOperation.DIFF_INSERT) {
            ...
            diffType === DiffOperation.DIFF_DELETE &&
  ...
  private match_bitap_(text: string, pattern: string, loc: number): number {
    ...
    if (bestLoc !== -1) {
      scoreThreshold = math.min(
        this.match_bitapScore_(0, bestLoc, pattern, loc),
        scoreThreshold,
      );
    ...
    for (let d = 0; d < pattern.length; d++) {
      ...
      while (binMin < binMid) {
        if (
          this.match_bitapScore_(d, loc + binMid, pattern, loc) <=
          scoreThreshold
        ) {
          binMin = binMid;
        } else {
